...
class CAN(object):
    def __init__(self, sess, is_sample=False):
        self.sess = sess
        ...
        if not self.is_sample and (not os.path.exists(self.data_dir) or not self.data):
            # print(self.data)
            print('\nPROCESS END')
    ...
    def build_model(self):
        ...
        correct_prediction = tf.equal(
            tf.argmax(self.y, 1), tf.argmax(self.discriminator_police_class, 1))
    ...
    def sampler(self):
        ...
        #     0, 255, [self.batch_size, self.random_noise_dim]).astype(np.float32)
        random_noise = np.random.rand(self.batch_size, self.random_noise_dim)
        print(f'Noise min: {np.min(random_noise)} and max: {np.max(random_noise)}')
        samples = self.sess.run(self.generator, feed_dict={
                                self.random_noise: random_noise})
        print(f'Sample -> min :{np.min(samples)} and max: {np.max(samples)}')
        os.makedirs(os.path.join(os.getcwd(), 'test_images'), exist_ok=True)
        for i in range(samples.shape[0]):
            save_single_image(samples[i, :, :, :],
                              os.path.join(os.getcwd(), f'test_images/test_no_{i}.png'))
