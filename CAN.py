...
from utils import save_single_image, save_images, get_image
from ops import deconv2d, conv2d, get_y, linear, lrelu, batch_norm, checkpoint_load, checkpoint_save
...
class CAN(object):
    def __init__(self, sess):
        ...
        self.csv_file_path = "data/wikiart/all_data_info_utf.csv"
    ...
    def build_model(self):
        ...
        self.y = tf.compat.v1.placeholder(
            tf.float32, [None, self.label_dim], name='y')
        self.real_image = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.input_size, self.input_size, 3],
                                                   name='real_images')
        self.random_noise = tf.compat.v1.placeholder(
            tf.float32, [None, self.random_noise_dim], name='random_noise')

        # tensorboard
        self.random_noise_summary = tf.compat.v1.summary.histogram(
        ...
        # generator summary
        self.generator_summary = tf.compat.v1.summary.image(
        ...
        correct_prediction = tf.equal(tf.argmax(input=self.y, axis=1), tf.argmax(
            input=self.discriminator_police_class, axis=1))
        ...
        self.generator_loss = self.generator_loss_fake + \
            1.0 * self.generator_loss_class_fake
        #
        self.discriminator_loss = self.discriminator_police_loss + \
            self.discriminator_thief_loss + self.discriminator_loss_class_real  # 1 + 0 + 1 = 2
        ...
        self.generator_loss_summary = tf.compat.v1.summary.scalar(
            "generator_loss_summary", self.generator_loss)
        # g_loss_sum
        self.discriminator_loss_summary = tf.compat.v1.summary.scalar(
    ...
    def train(self):
        ...
        checkpoint_dir_path = os.path.join(
            self.checkpoint_dir, self.checkpint_dir_model)
        ...
        for epoch in xrange(self.epoch):
            ...
            for index in xrange(0, batch_index):
                ...
                batch_images_path = self.data[index *
                ...
                batch_random_noise = np.random.normal(
                    0, 1, [self.batch_size, self.random_noise_dim]).astype(np.float32)
                ...
                if np.mod(counter, 300) == 1:
                    try:
                        ...
                        samples = self.sess.run(
                            self.generator(self.random_noise))
    ...
    def discriminator(self, input_, reuse=False):
        with tf.compat.v1.variable_scope("discriminator") as scope:
            ...
            shape = np.product(
                discriminator_layer5.get_shape()[1:].as_list())  #
            discriminator_layer6 = tf.reshape(
                discriminator_layer5, [-1, shape])  #
            discriminator_output = linear(
                discriminator_layer6, 1, 'd_ro_lin')  # (?, 1)

            discriminator_layer7 = lrelu(
                linear(discriminator_layer6, 1024, 'd_h8_lin'))  #
            discriminator_layer8 = lrelu(
                linear(discriminator_layer7, 512, 'd_h9_lin'))  #
            discriminator_class_output = linear(
                discriminator_layer8, self.label_dim, 'd_co_lin')  #
            discriminator_class_output_softmax = tf.nn.softmax(
                discriminator_class_output)  # (?, self.label_dim)
    ...
    def generator(self, random_noise):
        with tf.compat.v1.variable_scope("generator") as scope:
            ...
            generator_linear = linear(
                random_noise, 64 * 4 * 4 * 16, 'g_h0_lin')
            ...
            generator_layer1 = tf.nn.relu(batch_norm(
                generator_layer1, 'g_bn1'))  # (?, 8, 8, 1024)
            ...
            generator_layer2 = tf.nn.relu(batch_norm(
                generator_layer2, 'g_bn2'))  # (?, 16, 16, 512)
            ...
            generator_layer3 = tf.nn.relu(batch_norm(
                generator_layer3, 'g_bn3'))  # (?, 32, 32, 256)
    ...
    def sampler(self, random_noise):
        with tf.compat.v1.variable_scope("generator", reuse=True) as scope:
            ...
            # ([?, 100], 16,384])
            sampler_linear = linear(random_noise, 64 * 4 * 4 * 16, 'g_h0_lin')
