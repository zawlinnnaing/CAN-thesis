...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.real_image = tf.placeholder(
            tf.float32, [64, 256, 256, 3], name='real_images')
        self.random_noise = tf.placeholder(
            tf.float32, [None, 100], name='random_noise')

        # tensorboard
        self.random_noise_summary = tf.summary.histogram(
            "random_noise_summary", self.random_noise)

        # model build
        # Creating generator / discriminator
        self.generator = self.generator(self.random_noise)
        self.discriminator_police_sigmoid, self.discriminator_police = self.discriminator(
            self.real_image, reuse=False)
        self.discriminator_thief_sigmoid, self.discriminator_thief = self.discriminator(
        ...
        # discriminator cost
        self.discriminator_loss = self.discriminator_police_loss + \
            self.discriminator_thief_loss

        # tensorboard
        self.generator_loss_summary = tf.summary.scalar(
            "generator_loss_summary", self.generator_loss)
        self.discriminator_loss_summary = tf.summary.scalar(
    ...
    def train(self):
        ...
        checkpoint_dir_path = os.path.join(
            drive_checkpoint_dir, checkpoint_model)
        could_load, checkpoint_counter = checkpoint_load(
            self.sess, self.saver, drive_checkpoint_dir, checkpoint_model)
        if could_load:
        ...
        for epoch in xrange(35):
            ...
            for index in xrange(0, batch_index):
                ...
                batch_random_noise = np.random.uniform(
                    -1, 1, [64, 100]).astype(np.float32)
                ...
                errD_fake = self.discriminator_thief_loss.eval(
                    {self.random_noise: batch_random_noise})
                errD_real = self.discriminator_police_loss.eval(
                ...
                if np.mod(counter, 100) == 1:
                    try:
                        samples, d_loss, g_loss = self.sess.run(
                            [self.sampler, self.discriminator_loss,
                                self.generator_loss],
                        ...
                        print("[SAVE IMAGE] d_loss: %.8f, g_loss: %.8f" %
                              (d_loss, g_loss))
                ...
                if np.mod(counter, 500) == 1:
                    ...
                    checkpoint_save(self.sess, self.saver,
                                    checkpoint_dir_path, counter)
    ...
    def discriminator(self, input_, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            # [256, 256, 3], 32 => (128, 128, 32)
            discriminator_layer0 = lrelu(conv2d(input_, 32, name='d_h0_conv'))
            ...
            discriminator_layer6 = tf.reshape(
                discriminator_layer5, [-1, shape])
            discriminator_output = linear(
                discriminator_layer6, 1, 'd_ro_lin')  # (?, 1)
            ...
            # (64, 1)
            return tf.nn.sigmoid(discriminator_output), discriminator_output

    def generator(self, random_noise):
        with tf.variable_scope("generator") as scope:
            # (?, 100), 16,384 -> (100, 16,384)
            generator_linear = linear(
                random_noise, 64 * 16 * 4 * 4, 'g_h0_lin')
            generator_reshape = tf.reshape(
                generator_linear, [-1, 4, 4, 64 * 16])  # (?, 4, 4, 1024)
            generator_input = tf.nn.relu(batch_norm(
                generator_reshape, 'g_bn0'))  # (?, 4, 4, 1024)

            generator_layer1 = deconv2d(
                generator_input, [64, 8, 8, 64 * 16], name='g_layer1')  # (?, 8, 8, 512)
            generator_layer1 = tf.nn.relu(batch_norm(
                generator_layer1, 'g_bn1'))  # (?, 8, 8, 512)

            generator_layer2 = deconv2d(
                generator_layer1, [64, 16, 16, 64 * 8], name='g_layer2')  # (?, 16, 16, 256)
            generator_layer2 = tf.nn.relu(batch_norm(
                generator_layer2, 'g_bn2'))  # (?, 16, 16, 256)

            generator_layer3 = deconv2d(
                generator_layer2, [64, 32, 32, 64 * 4], name='g_layer3')  # (?, 32, 32, 128)
            generator_layer3 = tf.nn.relu(batch_norm(
                generator_layer3, 'g_bn3'))  # (?, 32, 32, 128)

            generator_layer4 = deconv2d(
                generator_layer3, [64, 64, 64, 64 * 2], name='g_layer4')  # (?, 64, 64, 128)
            generator_layer4 = tf.nn.relu(batch_norm(
                generator_layer4, 'g_bn4'))  # (?, 64, 64, 128)

            generator_layer5 = deconv2d(
                generator_layer4, [64, 128, 128, 64], name='g_layer5')  # (?, 128, 128, 64)
            generator_layer5 = tf.nn.relu(batch_norm(
                generator_layer5, 'g_bn5'))  # (?, 128, 128, 64)

            generator_output = deconv2d(
                generator_layer5, [64, 256, 256, 3], name='g_output')  # (?, 256, 256, 3)
    ...
    def sampler(self, random_noise):
        with tf.variable_scope("generator") as scope:
            ...
            # (?, 100), 16,384 -> (100, 16,384)
            sampler_linear = linear(random_noise, 64 * 16 * 4 * 4, 'g_h0_lin')
            sampler_reshape = tf.reshape(
                sampler_linear, [-1, 4, 4, 64 * 16])  # (?, 4, 4, 1024)
            sampler_input = tf.nn.relu(batch_norm(
                sampler_reshape, 'g_bn0', train=False))  # (?, 4, 4, 1024)

            sampler_layer1 = deconv2d(
                sampler_input, [64, 8, 8, 64 * 16], name='g_layer1')  # (?, 8, 8, 512)
            sampler_layer1 = tf.nn.relu(batch_norm(
                sampler_layer1, 'g_bn1', train=False))  # (?, 8, 8, 512)

            sampler_layer2 = deconv2d(
                sampler_layer1, [64, 16, 16, 64 * 8], name='g_layer2')  # (?, 16, 16, 256)
            sampler_layer2 = tf.nn.relu(batch_norm(
                sampler_layer2, 'g_bn2', train=False))  # (?, 16, 16, 256)

            sampler_layer3 = deconv2d(
                sampler_layer2, [64, 32, 32, 64 * 4], name='g_layer3')  # (?, 32, 32, 128)
            sampler_layer3 = tf.nn.relu(batch_norm(
                sampler_layer3, 'g_bn3', train=False))  # (?, 32, 32, 128)

            sampler_layer4 = deconv2d(
                sampler_layer3, [64, 64, 64, 64 * 2], name='g_layer4')  # (?, 64, 64, 128)
            sampler_layer4 = tf.nn.relu(batch_norm(
                sampler_layer4, 'g_bn4', train=False))  # (?, 64, 64, 128)

            sampler_layer5 = deconv2d(
                sampler_layer4, [64, 128, 128, 64], name='g_layer5')  # (?, 128, 128, 64)
            sampler_layer5 = tf.nn.relu(batch_norm(
                sampler_layer5, 'g_bn5', train=False))  # (?, 128, 128, 64)

            sampler_output = deconv2d(
                sampler_layer5, [64, 256, 256, 3], name='g_output')  # (?, 256, 256, 3)
