...
def batch_norm(inputs, scope_name, train=True):
    return tf.contrib.layers.batch_norm(inputs, decay=0.9, updates_collections=tf.compat.v1.GraphKeys.UPDATE_OPS, epsilon=1e-5, scale=True,
...
def conv2d(input_, output_dim, stddev=0.02, name="conv2d"):
    with tf.compat.v1.variable_scope(name):
        ...
        conv_w = tf.compat.v1.get_variable('conv_w', [4, 4, input_.get_shape()[-1], output_dim],
...
def deconv2d(input_, output_shape, stddev=0.02, name="deconv2d"):
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        deconv_w = tf.compat.v1.get_variable('deconv_w', [5, 5, output_shape[-1], input_shape[-1]],
...
def linear(input_, output_size, scope=None, stddev=0.02, ):
    ...
    with tf.compat.v1.variable_scope(scope or "Linear", reuse=tf.compat.v1.AUTO_REUSE):
        matrix = tf.compat.v1.get_variable("Matrix", [shape[1], output_size],
